...
registerWhen(register('step', () => {
    ...
    World.getAllTileEntitiesOfType(TileEntityChest.class).forEach((chest) => {
        if (chest.tileEntity.field_145989_m === 0)  // lidAngle
            chests.push(chest);
    });
}).setFps(2), () => settings.powderchesthighlight && getCurrentArea() === 'Crystal Hollows', { type: 'step', name: 'Powder Chest Highlight' });
...
}), () => settings.powderchesthighlight && getCurrentArea() === 'Crystal Hollows', { type: 'renderWorld', name: 'Powder Chest Highlight' });